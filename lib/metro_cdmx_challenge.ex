defmodule MetroCdmxChallenge do
  @moduledoc """
  The MetroCMDXChallenge create a graph to search the shortes-path and the route that 
  you must follow, from one station to other station.
  """
  defmodule Station do
    @moduledoc """
    The Station Module is a struct that storage name and coordinates of a staion.
    """
  ...
  defmodule Line do
    @moduledoc """
    The Line Module is a struct that storage name and coordinates of a Line.
    """
    defstruct [:name, :stations]
  end

  #  "./data/metro.kml"
  def metrolines(path) do
    import SweetXml
    # cargamos el archivo
    xml = File.read!(path)
    ...
    coord_station =
      xml
      |> xpath(~x"//Document/Folder[2]/Placemark/Point/coordinates/text()"sl)
  ...
  def metro_graph(path) do
    ...
    chunkeado =
      ...
      |> Enum.map(fn x ->
        x
        # puede que no halla pares, entonces tomamos el ultimo
    ...
    chunkeado
    |> Enum.reduce(graph, fn ypair, graph ->
